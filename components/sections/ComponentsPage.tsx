...
export function ComponentsPage() {
  ...
  const buttonVariants = [
    { label: t('Primary', 'Primary'), cls: 'bg-[#2196F3] text-white', token: '#2196F3', tokenCls: 'bg-[#2196F3]/10 text-brand.dark-blue', desc: t('primaryBtnDesc', 'Use for the single primary action per view (save, submit, confirm).'), code: '<button className="bg-[#2196F3] text-white px-4 py-2 rounded-lg font-semibold">Primary</button>' },
    { label: t('Accent CTA', 'Accent CTA'), cls: 'bg-[#FFC107] text-[#0D0F12]', token: '#FFC107', tokenCls: 'bg-[#FFC107]/15 text-[#92610B]', desc: t('High-attention call-to-action, one per view', 'High-attention call-to-action, one per view'), code: '<button className="bg-[#FFC107] text-[#0D0F12] px-4 py-2 rounded-lg font-semibold">Accent CTA</button>' },
    { label: t('Ghost', 'Ghost'), cls: 'bg-white text-[#2196F3] border-2 border-[#2196F3]', token: '2px border', tokenCls: 'bg-[#2196F3]/10 text-brand.dark-blue', desc: t('Tertiary, low visual weight', 'Tertiary, low visual weight'), code: '<button className="bg-white text-[#2196F3] border-2 border-[#2196F3] px-4 py-2 rounded-lg font-semibold">Ghost</button>' },
    { label: t('Destructive', 'Destructive'), cls: 'bg-[#DC2626] text-white', token: '#DC2626', tokenCls: 'bg-[#FEE2E2] text-[#DC2626]', desc: t('destructiveBtnDesc', 'Destructive actions — delete, remove, discard. Always confirm before executing.'), code: '<button className="bg-[#DC2626] text-white px-4 py-2 rounded-lg font-semibold">Destructive</button>' },
    { label: t('Disabled', 'Disabled'), cls: 'bg-[#F0F0F0] text-[#ADADAD] cursor-not-allowed', token: 'opacity 40%', tokenCls: 'bg-[#F0F0F0] text-[#ADADAD]', desc: t('Non-interactive state', 'Non-interactive state'), disabled: true, code: '<button className="bg-[#F0F0F0] text-[#ADADAD] px-4 py-2 rounded-lg font-semibold cursor-not-allowed" disabled>Disabled</button>' },
  ]
  ...
  const cardVariants = [
    {
      ...
      extra: null,
      cls: 'border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.06)]',
    },
    {
      ...
      extra: null,
      cls: 'border border-[#E8E8E8] shadow-[0_8px_24px_rgba(0,0,0,0.1)] -translate-y-[1px]',
    },
    {
  ...
  return (
    <div>
      ...
      <section className="mb-10">
        ...
        <div className="bg-white rounded-xl border border-[#E8E8E8] overflow-hidden divide-y divide-[#F0F0F0]">
          {buttonVariants.map((btn) => (
            <div key={btn.label} className="flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-4 transition-colors hover:bg-[#FAFAFA]">
              ...
              <div className="flex-1 min-w-0 hidden sm:block">
                <p className="text-[13px] font-semibold text-[#1A1A1A]">{btn.label}</p>
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-medium text-[#9E9E9E] hover:text-[#333333] hover:bg-[#F0F0F0] active:scale-95 transition-all shrink-0"
              >
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { label: t('Default', 'Default'), border: 'border-[#E0E0E0]', val: '', ph: t('e.g. Spring Campaign 2025', 'e.g. Spring Campaign 2025'), note: t('Resting state', 'Resting state') },
            ...
            { label: t('Disabled', 'Disabled'), border: 'border-[#E8E8E8]', val: t('Read-only value', 'Read-only value'), ph: '', note: t('Non-interactive', 'Non-interactive'), disabled: true },
          ].map((inp) => (
            <div key={inp.label} className="bg-white rounded-xl border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.06)] p-5">
              ...
              <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('Campaign name', 'Campaign name')}</label>
              <input
                readOnly disabled={inp.disabled}
                value={inp.val} placeholder={inp.ph}
                className={`w-full px-3 py-2.5 text-sm rounded-lg border outline-none transition-all bg-white text-[#333333] placeholder:text-brand.muted-steel ${inp.disabled ? 'bg-[#F7F7F7] text-brand.muted-steel cursor-not-allowed' : ''} ${inp.border}`}
              />
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {/* Live demo */}
          <div className="bg-white rounded-xl border border-[#E8E8E8] p-7">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('First name', 'First name')}</label>
                  <input readOnly value={t('Yasser', 'Yasser')} className="w-full px-3 py-2.5 text-sm rounded-lg border border-[#E0E0E0] bg-white text-[#333333] outline-none" />
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('Last name', 'Last name')}</label>
                  <input readOnly value={t('Dorgham', 'Dorgham')} className="w-full px-3 py-2.5 text-sm rounded-lg border border-[#E0E0E0] bg-white text-[#333333] outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('Email address', 'Email address')}</label>
                <input readOnly value={t('hello@mediabubble.com', 'hello@mediabubble.com')} className="w-full px-3 py-2.5 text-sm rounded-lg border border-[#2196F3] ring-2 ring-[#2196F3]/20 bg-white text-[#333333] outline-none" />
              </div>
              <div>
                <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('Service', 'Service')}</label>
                <div className="relative">
                  <select className="w-full px-3 py-2.5 text-sm rounded-lg border border-[#E0E0E0] bg-white text-[#333333] outline-none appearance-none cursor-pointer">
              ...
              <div>
                <label className="block text-[12px] font-medium text-[#444444] mb-1.5">{t('Message', 'Message')}</label>
                <textarea readOnly rows={3} value={t('Looking forward to working with your team on our rebrand.', 'Looking forward to working with your team on our rebrand.')} className="w-full px-3 py-2.5 text-sm rounded-lg border border-[#E0E0E0] bg-white text-[#333333] outline-none resize-none leading-relaxed" />
              </div>
          ...
          {/* Specs */}
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.06)] divide-y divide-[#F0F0F0]">
              {[
                ...
                <div key={r.rule} className="flex items-start gap-4 px-5 py-3.5">
                  ...
                  <div>
                    <code className="text-[11px] font-mono text-[#333333]">{r.val}</code>
                    <p className="text-[11px] text-brand.muted-steel mt-0.5">{r.note}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="bg-[#EBF5FB] rounded-xl border border-[#BBDEFB] p-5">
              <p className="text-[12px] font-semibold text-brand.dark-blue mb-1.5">{t('Validation timing', 'Validation timing')}</p>
              <p className="text-[12px] text-[#666666] leading-relaxed">{t('validationDesc', 'Validate inputs on blur, not on every keystroke. Display error messages directly below the field that failed. Validating on every keystroke frustrates users.')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          {cardVariants.map((c) => (
            <div key={c.title} className={`rounded-xl overflow-hidden ${c.extra || ''} ${c.cls}`}>
              <div className={`p-5 ${c.extra?.includes('bg-[#072A6B]') ? '' : 'bg-white'}`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className={`text-[13px] font-bold ${c.extra?.includes('bg-[#072A6B]') ? 'text-white' : 'text-[#1A1A1A]'}`}>{c.title}</p>
                    <p className={`text-[11px] font-mono mt-0.5 ${c.extra?.includes('bg-[#072A6B]') ? 'text-white/40' : 'text-brand.muted-steel'}`}>{c.subtitle}</p>
                  </div>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md ${c.extra?.includes('bg-[#072A6B]') ? 'bg-white/10 text-white/50' : 'bg-[#F4F6F9] text-[#9E9E9E]'}`}>{t('card', 'card')}</span>
                </div>
                <p className={`text-[13px] leading-relaxed ${c.extra?.includes('bg-[#072A6B]') ? 'text-white/65' : 'text-[#666666]'}`}>{c.desc}</p>
        ...
        <div className="bg-[#F4F6F9] rounded-xl divide-y divide-[#E8E8E8]">
          {[
            ...
            <div key={item.token} className="flex items-start gap-4 px-5 py-3 transition-colors hover:bg-black/[0.02]">
              ...
              <p className="text-[12px] font-semibold text-[#333333] w-48 shrink-0">{item.value}</p>
      ...
      <section className="mb-10">
        ...
        <div className="bg-white rounded-xl border border-[#E8E8E8] divide-y divide-[#F0F0F0]">
          {[
            { label: t('Underline tabs', 'Underline tabs'), type: t('primary', 'primary'), typeColor: 'bg-[#2196F3]/10 text-brand.dark-blue', desc: t('Standard nav — active state uses bottom border + bold weight. Best for top-level navigation.', 'Standard nav — active state uses bottom border + bold weight. Best for top-level navigation.'), code: '<div className="flex gap-6 border-b border-[#E8E8E8] px-1">\n  <button className="text-[13px] font-semibold text-[#333333] border-b-2 border-[#2196F3] pb-2">Active</button>\n  <button className="text-[13px] font-medium text-brand.muted-steel pb-2 hover:text-[#333333]">Inactive</button>\n</div>', active: true },
            { label: t('Pill tabs', 'Pill tabs'), type: t('accent', 'accent'), typeColor: 'bg-[#FFC107]/15 text-[#92610B]', desc: t('Content-switching pattern — active pill uses brand blue fill. Used in sub-navigation and filter bars.', 'Content-switching pattern — active pill uses brand blue fill. Used in sub-navigation and filter bars.'), code: '<div className="flex gap-2">\n  <button className="px-4 py-1.5 rounded-full bg-[#2196F3] text-white text-[12px] font-semibold">Active</button>\n  <button className="px-4 py-1.5 rounded-full bg-[#F4F6F9] text-[#666666] text-[12px] font-medium hover:bg-[#E8E8E8]">Inactive</button>\n</div>', active: false },
            { label: t('Segmented control', 'Segmented control'), type: t('data', 'data'), typeColor: 'bg-brand.dark-blue/10 text-[#072A6B]', desc: t('Toggle between views. Equal-width segments, active state slides between them.', 'Toggle between views. Equal-width segments, active state slides between them.'), code: '<div className="inline-flex rounded-lg bg-[#F4F6F9] p-0.5">\n  <button className="px-4 py-1.5 rounded-md bg-white text-[#333333] text-[12px] font-semibold shadow-sm">Day</button>\n  <button className="px-4 py-1.5 rounded-md text-brand.muted-steel text-[12px] font-medium">Week</button>\n  <button className="px-4 py-1.5 rounded-md text-brand.muted-steel text-[12px] font-medium">Month</button>\n</div>', active: true },
          ].map((tab, i) => (
            <div key={tab.label} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 px-5 sm:px-6 py-4 transition-colors hover:bg-[#FAFAFA]">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  {tab.active ? (
                    <>
                      <div className="flex gap-4 sm:gap-6 border-b border-[#E8E8E8] px-1">
                        <button className="text-[13px] font-semibold text-[#333333] border-b-2 border-[#2196F3] pb-2">{t('Active', 'Active')}</button>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="px-4 py-1.5 rounded-full bg-[#F4F6F9] text-[#666666] text-[12px] font-medium">{t('Inactive', 'Inactive')}</button>
                      <button className="px-4 py-1.5 rounded-full bg-[#F4F6F9] text-[#666666] text-[12px] font-medium">{t('Tab', 'Tab')}</button>
                    </div>
                  )}
                </div>
      ...
      <section className="mb-16">
        ...
        <div className="bg-white rounded-xl border border-[#E8E8E8] overflow-hidden">
          <div className="p-5 sm:p-6 border-b border-[#F0F0F0]">
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                { label: t('Draft', 'Draft'), cls: 'bg-[#F4F6F9] text-[#666666]' },
              ].map((badge) => (
          ...
          <div className="border-b border-[#F0F0F0] px-5 sm:px-6 py-4 flex items-center justify-between">
            <div>
              <p className="text-[12px] font-semibold text-[#333333] mb-1">{t('Token badges', 'Token badges')}</p>
              <div className="flex flex-wrap gap-2">
                ...
                <span className="px-2 py-0.5 rounded-md bg-[#F4F6F9] text-[#666666] text-[10px] font-mono font-semibold">#333333</span>
              </div>
            </div>
            <button
          ...
          <div className="px-5 sm:px-6 py-4 flex items-center justify-between">
            <div>
              <p className="text-[12px] font-semibold text-[#333333] mb-1">{t('Count badges', 'Count badges')}</p>
              <div className="flex items-center gap-4">
                <span className="relative inline-flex">
                  <Bell size={16} className="text-[#666666]" />
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-[#DC2626] text-white text-[10px] font-bold flex items-center justify-center">3</span>
                </span>
