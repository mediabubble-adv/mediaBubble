...
export function PageHero({ kicker, title, titleHighlight, description, showLogo = false, icon: Icon, stats }: PageHeroProps) {
  ...
  return (
    <div className="bg-[#072A6B] relative">
      ...
      <div className="px-10 sm:px-16 lg:pl-20 lg:pr-[60px] py-16 sm:py-24 max-w-[1400px] mx-auto">
      ...
      {stats && stats.length > 0 && (
        <div className="border-t border-white/[0.06]">
          <div className="grid grid-cols-2 sm:grid-cols-4 max-w-[1400px] mx-auto">
            {stats.map((stat, i) => (
              <div key={stat.label} className={`px-6 lg:px-10 py-4 ${i < stats.length - 1 ? 'border-r border-white/[0.06]' : ''}`}>
