    ...
    <div>
      ...
      <div className="px-6 lg:px-10 py-8 lg:py-12 max-w-[1400px] mx-auto">
        ...
        {fonts.map((font) => (
          <div
            key={font.name}
            className="bg-white rounded-xl border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.07)] transition-all duration-150 overflow-hidden flex flex-col"
          ...
          >
            <div className="p-5 pb-0">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-display text-sm font-bold text-[#333333]">{font.name}</h3>
                  <TooltipHint text={font.sub} />
                  <p className="text-[11px] text-[#9E9E9E] font-medium mt-0.5">{font.role}</p>
                </div>
                <span className="text-[10px] font-mono text-[#9E9E9E] bg-[#F5F5F5] px-2 py-1 rounded-md">{font.cssVar}</span>
              </div>

              <div className={`relative py-6 px-3 rounded-lg mb-4 ${font.name === 'JetBrains Mono' ? 'bg-[#0D0F12]' : 'bg-[#F5F5F5]'}`}>
                <p
                  className={`${font.name === 'JetBrains Mono' ? 'font-mono text-[#FFC107]' : font.name === 'Cairo' ? 'font-arabic text-brand.dark-blue' : 'font-display text-[#333333]'} text-2xl font-bold truncate transition-all ${hoveredCard === font.name ? 'opacity-100' : 'opacity-90'}`}
                  dir={font.name === 'Cairo' ? 'rtl' : undefined}
                >
                  {font.specimen}
                </p>
                <p className={`text-[10px] font-mono mt-2 ${font.name === 'JetBrains Mono' ? 'text-white/30' : 'text-[#9E9E9E]'}`}>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {font.weights.map((w) => (
                  <span
                    key={w.weight}
                    className={`px-2 py-1 rounded-md text-[10px] font-mono transition-all ${hoveredCard === font.name ? 'bg-[#2196F3]/10 text-[#2196F3]' : 'bg-[#F5F5F5] text-[#666]'}`}
                  >
                    {w.label} {w.weight}
                  </span>
                ))}
            ...
            <div className="px-5 pb-4 mt-auto">
              <p className="text-sm text-[#666666] leading-relaxed mb-3">{font.sub}</p>
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.07)] overflow-hidden">
          {scaleSteps.map((step, i) => (
            <div
              key={step.token}
              className={`flex items-center gap-5 px-6 py-[18px] transition-all duration-150 hover:bg-black/[0.015] active:bg-black/[0.03] ${i < scaleSteps.length - 1 ? 'border-b border-[#E8E8E8]' : ''} ${i === 5 ? 'border-t-2 border-t-[#E8E8E8]' : ''}`}
            >
              <div className="w-28 shrink-0 flex items-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-semibold text-[#333333] leading-tight">{step.token}</p>
                  <p className="text-[10px] font-mono text-[#9E9E9E] mt-px">{step.size}</p>
                </div>
              </div>
              <div className="flex-1 min-w-0">
                <p className={step.cls}>{step.sample}</p>
              </div>
              <div className="hidden md:flex items-center gap-4 text-[10px] font-mono text-[#9E9E9E] shrink-0">
              ...
              <div className="w-[140px] shrink-0 text-end hidden lg:block">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-mono text-[#9E9E9E] hover:text-[#333333] hover:bg-[#F0F0F0] active:scale-95 transition-all"
                >
          ...
          {[
            {
              ...
              headingCls: 'font-display text-2xl sm:text-3xl font-bold tracking-tight text-brand.dark-blue',
              ...
              bodyCls: 'font-sans text-sm leading-relaxed text-[#666666]',
              from: 'Poppins 700',
              to: 'Inter 400',
            },
            {
              ...
              headingCls: 'font-display text-xl font-semibold text-brand.dark-blue',
            ...
            },
            {
              ...
              headingCls: 'font-display text-base font-semibold text-brand.dark-blue',
              ...
              bodyCls: 'font-sans text-[13px] leading-snug text-[#666666]',
              from: 'Poppins 600',
              to: 'Inter 400',
            },
          ].map((pair) => (
            <div key={pair.context} className="bg-white rounded-xl border border-[#E8E8E8] shadow-[0_2px_6px_rgba(0,0,0,0.07)] p-5 transition-all duration-150">
              ...
              <div className="flex items-center gap-2 mt-4 pt-3 border-t border-[#E8E8E8]">
                <span className="text-[10px] font-mono bg-[#F5F5F5] px-1.5 py-0.5 rounded text-[#888]">{pair.from}</span>
                <ArrowRight size={10} className="text-[#BBB]" />
                <span className="text-[10px] font-mono bg-[#F5F5F5] px-1.5 py-0.5 rounded text-[#888]">{pair.to}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger-grid">
          {[
            ...
            <div key={item.label} className="bg-[#F4F6F9] rounded-xl p-5 transition-all duration-150 hover:bg-[#EEF1F6]">
              <p className="text-[12px] font-semibold text-brand.dark-blue mb-1">{item.label}</p>
              <p className="text-[22px] font-display font-bold text-[#333333] tracking-tight">{item.detail}</p>
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={r.rule} className="bg-[#F4F6F9] rounded-xl p-5 transition-all duration-150 hover:bg-[#EEF1F6]">
