...
import { taxonomyTree, branchColors, countLeaves } from '@/data/arabic-taxonomy'
import { SkillTreeNode } from './SkillTreeNode'
import { useDebounce } from '@/utils/use-debounce'
import { fuzzyMatchAny, fuzzyScore } from '@/utils/fuzzy-search'
        ...
        <div>
          ...
          <div className="space-y-0.5">
            <button
              ...
              className={`
                w-full text-left px-3 py-2 rounded-lg text-[12px] font-medium transition-all duration-150 active:scale-[0.98]
            ...
            >
              <span className="flex items-center gap-2">
                <Layers size={13} />
                All Branches
                <span className="ml-auto text-[10px] text-gray-400 tabular-nums">{totalLeaves}</span>
              </span>
            </button>
            {branchStats.map((b, i) => (
              <button
                ...
                className={`
                  w-full text-left px-3 py-2 rounded-lg text-[12px] font-medium transition-all duration-150 active:scale-[0.98] animate-fade-in-up-stagger
      ...
      <div className="space-y-4">
        <div className="relative animate-fade-in-up" style={{ animationDelay: '0.04s' }}>
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2.5 text-[13px] border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 transition-all bg-white"
          />
          {inputValue && (
