...
- RTL layouts: infinite-scroll marquees keep the animation track `dir="ltr"` for seamless `translateX` loop math; set per-card `dir` for Arabic text. Footer and contact phone use `tel:` from `resolveMarketSiteConfig().phone` (ASCII E.164) with `dir="ltr"`, `tabular-nums`, and `unicode-bidi: isolate`; keep Western digits in `ar` locale phone strings.
- Home hero is full viewport height on both market sites; inner pages use the shorter contact-page `PageHero` height (not full screen).
- GitHub README Mermaid diagrams must quote node labels containing `@` (bare `@` breaks GitHub's parser). The `mediabubble-adv/mediaBubble` repo is private—use a static shields.io CI badge linked to Actions, not the GitHub workflow status badge (404 on private repos).
