...
export function GettingStartedPage({ onNavigate }: { onNavigate: (id: string) => void }) {
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger-grid">
          {steps.map((s, i) => {
            ...
            return (
              <div
                key={s.step}
                className="relative bg-brand-surface rounded-xl border border-brand-whisper-border dark:border-brand-light-border p-5 flex flex-col"
              >
                ...
                <h3 className="font-display text-base font-bold text-brand-text mb-2">{s.title}</h3>
                <p className="text-sm text-brand-text-secondary leading-relaxed mb-4 flex-1">{s.description}</p>
                <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger-grid">
          {quickActions.map((a) => (
            <button
              ...
              className="bg-brand-surface rounded-xl border border-brand-whisper-border dark:border-brand-light-border px-5 py-5 text-start transition-all duration-150 group"
            >
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="bg-brand-surface rounded-xl border border-brand-whisper-border dark:border-brand-light-border p-5">
              <p className="text-sm font-bold text-brand-text mb-2">{item.q}</p>
              <p className="text-sm text-brand-text-secondary leading-relaxed">{item.a}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Pro Tip */}
      <div className="flex items-start gap-0 bg-brand-surface rounded-xl border border-brand-whisper-border dark:border-brand-light-border overflow-hidden">
        <div className="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-4 px-5 py-5">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-brand-text mb-1">{t('proTipTitle', 'Pro tip')}</p>
