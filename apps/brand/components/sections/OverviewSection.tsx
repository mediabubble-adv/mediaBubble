...
import { BrandDocCard, BrandInfoBand, brandDocCardShell, brandDocRowHover } from '@/components/ui/brand-doc'
  ...
  return (
    <div>
      ...
      <section className="mb-10">
        <div className="mb-4">
          <h2 className="text-[13px] font-semibold text-brand-dark-blue dark:text-brand-blue">{t('overview.guidelinesIndex', 'Guidelines Index')}</h2>
        </div>
          ...
          {navCards.map((card) => {
            ...
            return (
              <button
                ...
                className={`${brandDocCardShell} p-4 text-start transition-all duration-150 hover:shadow-[0_2px_8px_rgba(0,0,0,0.08)] dark:hover:shadow-black/20 hover:-translate-y-[1px] active:scale-[0.97] focus-visible:ring-2 focus-visible:ring-brand-yellow`}
              >
                ...
                <p className="text-[13px] font-semibold text-brand-text leading-tight mb-1">{card.label}</p>
      ...
      <section className="mb-10">
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h2 className="text-[13px] font-semibold text-brand-dark-blue dark:text-brand-blue">{t('overview.pillars.heading', 'Brand Pillars')}</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger-grid">
          {[
            ...
            <div key={p.nameKey} className={`${brandDocCardShell} overflow-hidden`}>
      ...
      <section className="mb-10">
        <BrandInfoBand className="mb-5">
          <h2 className="text-[13px] font-semibold text-brand-dark-blue dark:text-brand-blue">{t('overview.visualSystem.heading', 'Visual System at a Glance')}</h2>
        </BrandInfoBand>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* Colors */}
          <div className={`${brandDocCardShell} p-5`}>
            ...
            <div className="flex gap-2 mb-3">
              {['#2196F3', '#1565C0', '#FFC107', '#072A6B', '#333333', '#FAFAFA'].map((c) => (
                <div key={c} className="w-7 h-7 rounded-lg border border-brand-whisper-border dark:border-brand-light-border" style={{ backgroundColor: c }} title={c} />
              ))}
            </div>
          ...
          <div className={`${brandDocCardShell} p-5`}>
            ...
            <p className="font-sans text-sm text-brand-text-secondary leading-snug mb-3">{t('overview.visualSystem.bodyFont', 'Inter for body copy')}</p>
          ...
          {/* Icons */}
          <div className={`${brandDocCardShell} p-5`}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-brand-text-muted mb-3">{t('overview.visualSystem.iconography', 'Iconography')}</p>
            <div className="flex gap-2 mb-3">
              {([Search, Settings, Mail, Bell, User, ArrowRight] as React.ComponentType<{size: number; strokeWidth: number; className: string}>[]).map((Icon, i) => (
                <div key={i} className="w-8 h-8 rounded-lg bg-brand-canvas dark:bg-white/[0.04] flex items-center justify-center">
      ...
      <section className="mb-10">
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h2 className="text-[13px] font-semibold text-brand-dark-blue dark:text-brand-blue">{t('overview.services.heading', 'Service Areas')}</h2>
        </div>
        <div className="relative grid grid-cols-1 sm:grid-cols-2 gap-px bg-brand-whisper-border dark:bg-brand-light-border rounded-xl overflow-hidden">
          ...
          {brand.services.map((service, idx) => (
            <div key={service.name} className="flex items-start gap-3 px-5 py-4 bg-brand-surface transition-all duration-150 hover:bg-black/[0.02] dark:hover:bg-white/[0.04] active:bg-black/[0.04] dark:active:bg-white/[0.06]">
              ...
              <div>
                <p className="text-sm font-semibold text-brand-text">{t(`services.${idx}.name`, service.name)}</p>
                <p className="text-sm text-brand-text-secondary leading-relaxed mt-0.5">{t(`services.${idx}.desc`, service.desc)}</p>
              </div>
            </div>
          ))}
        </div>
