...
import type { InsightPost } from '@/lib/data/insights-posts'
import {
  formatReadingTime,
  getRelatedPosts,
  getTableOfContents,
  slugifyHeading,
} from '@/lib/data/insights-utils'
import { Container } from '@/components/layout/Container'
import { CtaSection } from '@/components/sections/CtaSection'
import { MainLayout } from '@/components/layout/MainLayout'
import { InsightsTableOfContents } from '@/components/features/insights/InsightsTableOfContents'
import { InsightsAuthorBio } from '@/components/features/insights/InsightsAuthorBio'
import { InsightsNewsletterCta } from '@/components/features/insights/InsightsNewsletterCta'
import { InsightsShareButtons } from '@/components/features/insights/InsightsShareButtons'
import { RelatedPostsSection } from '@/components/features/insights/RelatedPostsSection'

const CATEGORY_SERVICE: Record<string, { slug: string; label: string; description: string }> = {
  'SEO':              { slug: 'seo',      label: 'SEO & Organic Growth',   description: 'See exactly how we build rankings for Hurghada businesses.' },
  'Paid Ads':         { slug: 'ppc',      label: 'Paid Advertising (PPC)', description: 'Find out how we manage Google and Meta campaigns for local businesses.' },
  'Social Media':     { slug: 'social',   label: 'Social Media Marketing', description: 'Explore how we grow and monetise social audiences in the region.' },
  'Branding':         { slug: 'branding', label: 'Branding & Design',      description: 'See how we build brand identities that command premium pricing.' },
  'Web Development':  { slug: 'web',      label: 'Web Development',        description: 'Explore our web design and development service for regional businesses.' },
...
export function InsightPostContent({ post }: Props) {
            ...
            <div className="grid lg:grid-cols-[minmax(0,1fr)_260px] gap-10 xl:gap-14 items-start">
              ...
              <article className="min-w-0 max-w-3xl lg:max-w-none">
                ...
                <div className="lg:hidden mb-8">
                  <InsightsTableOfContents items={tocItems} />
                </div>
                ...
                <div className="mt-10 space-y-8">
                  <InsightsAuthorBio authorId={post.authorId ?? 'yasser'} />
                ...
                {shareUrl && (
                  <div className="mt-10 lg:hidden">
                    ...
                    <InsightsShareButtons title={post.title} url={shareUrl} />
                  </div>
                )}
              </article>
              ...
              <aside className="hidden lg:block space-y-6 sticky top-28 self-start">
                ...
                {shareUrl && (
                  <div>
                    ...
                    <InsightsShareButtons title={post.title} url={shareUrl} layout="vertical" />
                  </div>
                )}
              </aside>
