...
export function BlogNewsletterCta() {
  ...
  return (
    <aside className="rounded-2xl border border-brand-whisper-border bg-brand-navy p-6 sm:p-8 text-white">
      <div className="flex items-start gap-3 mb-4">
        <Mail size={20} className="text-brand-yellow shrink-0 mt-0.5" aria-hidden="true" />
        <div>
          <h3 className="font-display text-[18px] font-bold mb-1">Get marketing insights in your inbox</h3>
          <p className="text-[14px] text-white/75 leading-relaxed">
            Practical guides for tourism and hospitality brands in the UAE — no fluff, no spam.
          </p>
        </div>
      </div>

      {status === 'success' ? (
        <p className="text-[14px] font-medium text-brand-yellow">Thanks — you&apos;re subscribed.</p>
      ) : (
