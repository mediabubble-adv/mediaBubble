...
export function BlogGrid() {
  const { t, dir } = useI18n()
  const searchPlaceholderVariant = useExperiment('blogSearchPlaceholder')
  ...
  useEffect(() => {
    ...
    fetch(`/api/blog/search?${params.toString()}`, { signal: controller.signal })
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        ...
        setUseRemote(true)
        trackBlogSearch(debouncedSearch, data.total)
      })
      .catch(error => {
  ...
  const filtered = useMemo(() => {
    ...
    return searchBlogPosts(BLOG_POSTS, {
      query: search,
  ...
  function selectCategory(next: BlogCategory) {
    setCategory(next)
    setPage(1)
  }
