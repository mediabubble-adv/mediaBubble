...
```html
<div class="newsletter-modal newsletter-modal--hidden" role="dialog" aria-labelledby="modal-title">
...
class NewsletterModal {
  ...
  show() { /* fade in, focus management */ }
  hide() { /* fade out, cleanup */ }
  submit(email) { /* HubSpot API + GA4 event */ }
  setupExitIntent() { /* mouse leave detection */ }
}
```
...
class ResponsiveMenu {
  ...
  openMenu() { /* show, manage focus */ }
  closeMenu() { /* hide, restore focus */ }
  setupKeyboardNav() { /* Tab, Arrow, Enter, Esc */ }
  setupTouchGestures() { /* swipe, tap */ }
  handleResize() { /* switch between desktop/mobile */ }
}
```
...
class InteractiveCursor {
  ...
  init() { /* create cursor element */ }
  onMouseMove(e) { /* update position */ }
  onHoverElement(isHovering) { /* expand/contract */ }
  destroy() { /* cleanup on touch devices */ }
}
```
