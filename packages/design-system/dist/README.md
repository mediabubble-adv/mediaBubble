...
| **pnpm 9+** *(optional)* | Local dev via `pnpm-workspace.yaml`; do not run `npm install` inside `apps/*` |
| **Supabase project** | Required only for **MediaBubble Launcher** (Postgres URLs) |
| **Vercel CLI** *(optional)* | Env pull and deploys |
...
### 4. MediaBubble Launcher *(internal ops)*
...
- **MediaBubble Launcher** — Phase 2 modules shipping (time, CRM, AI, chat, automation, campaigns); production DNS + env on Vercel (see `LAUNCHER_PLAN_V2.md`)
