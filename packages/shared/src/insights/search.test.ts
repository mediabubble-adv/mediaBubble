import { searchInsightsPosts, toInsightSearchSummary } from './search'
...
describe('searchInsightsPosts', () => {
  it('returns all posts when query and category are empty', () => {
    const { results, total } = searchInsightsPosts(SAMPLE_POSTS)
    expect(total).toBe(2)
    expect(results).toHaveLength(2)
  })

  it('filters by category', () => {
    const { results, total } = searchInsightsPosts(SAMPLE_POSTS, { category: 'SEO' })
    expect(total).toBe(1)
    expect(results[0]?.slug).toBe('seo-guide')
  })

  it('searches intro and section body text', () => {
    const { results, total } = searchInsightsPosts(SAMPLE_POSTS, { query: 'structured data' })
    expect(total).toBe(1)
    expect(results[0]?.slug).toBe('seo-guide')
  })

  it('applies limit after filtering', () => {
    const { results, total } = searchInsightsPosts(SAMPLE_POSTS, { limit: 1 })
    expect(total).toBe(2)
    expect(results).toHaveLength(1)
  })
})

describe('toInsightSearchSummary', () => {
  it('removes full-content fields from API payloads', () => {
    const summary = toInsightSearchSummary(SAMPLE_POSTS[0])
